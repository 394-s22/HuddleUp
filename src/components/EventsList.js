...
const EventsList = ({ events, searchText, userData, currentPage, filterCondition }) => {
  ...
  const joined_condition = (event) => {
    return user && userData && userData[user.uid].joined_events &&
      userData[user.uid].joined_events.includes(event.id);
  };

  const hosted_condition = (event) => {
    return user && userData && userData[user.uid].hosted_events &&
